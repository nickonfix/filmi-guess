...
export default function LandingPage() {
  ...
  return (
    <main className="relative flex min-h-screen flex-col overflow-hidden">
      {/* Atmospheric mesh gradient — occupies the top half of the page */}
      ...
      <header className="sticky top-0 z-30 border-b border-hairline/70 bg-canvas-soft/80 backdrop-blur-md">
        <nav className="mx-auto flex h-16 w-full max-w-page items-center justify-between px-4 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={openBrowse} className="nav-cta-ghost">Browse rooms</button>
            <button
              onClick={() => { if (!playerName.trim()) { setError('Enter your name'); setMode('home'); return; } setMode('create'); setError(''); }}
              className="nav-cta-signup"
            >
              Play now
            </button>
          </div>
      ...
      {/* Hero */}
      <section className="relative mx-auto w-full max-w-page px-4 pt-16 sm:px-6 sm:pt-24">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="body-lg mx-auto mt-6 max-w-xl text-body">
            ...
            {mode === 'browse' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={fetchRooms} className="text-sm font-medium text-link transition-colors hover:text-link-deep">↻ Refresh</button>
                </div>
                <h2 className="display-sm text-center">Public rooms</h2>
                {browseLoading ? (
                  <p className="py-10 text-center text-sm text-mute">Loading rooms…</p>
                ) : publicRooms.length === 0 ? (
                  <p className="py-10 text-center text-sm text-mute">No public rooms right now.<br />Create one and make it public!</p>
                ) : (
                  <div className="-mx-1 max-h-80 space-y-2 overflow-y-auto px-1">
                    {publicRooms.map(r => (
                      <button
                        ...
                        disabled={loading}
                        className="card w-full p-4 text-left transition-all hover:shadow-card-md disabled:opacity-50"
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-mono text-base font-medium tracking-[0.2em] text-ink">{r.code}</span>
                        ...
                        <div className="mt-2 flex gap-1.5 text-base">
                          {r.categories.map(c => {
      ...
      {/* How it works — 3-up feature cards */}
      ...
      <section className="relative overflow-hidden bg-primary">
        ...
        <div className="relative mx-auto w-full max-w-page px-4 py-24 sm:px-6 sm:py-32">
          <div className="mx-auto max-w-2xl text-center">
            <p className="eyebrow mb-4 text-white/50">Built for the group chat</p>
            <h2 className="display-lg text-on-primary">
            ...
            <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
              <button
                onClick={() => { if (!playerName.trim()) { setError('Enter your name'); setMode('home'); window.scrollTo({ top: 0, behavior: 'smooth' }); return; } setMode('create'); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                className="btn-secondary w-full sm:w-auto"
              >
                Create a room
              </button>
              <button
                onClick={() => { setMode('join'); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                className="btn w-full rounded-pill border border-white/15 px-6 text-base text-white/80 transition-colors hover:bg-white/10 sm:h-12 sm:w-auto"
              >
                Join with a code
              </button>
            </div>
          ...
          {/* Stat strip */}
          <div className="mx-auto mt-16 grid max-w-2xl grid-cols-3 gap-px overflow-hidden rounded-lg bg-white/10 shadow-card-dark">
            {[
              { value: '50', label: 'players / room' },
              { value: '4', label: 'cinema categories' },
              { value: '∞', label: 'free games' },
            ].map(s => (
              <div key={s.label} className="bg-primary px-4 py-6 text-center">
                <div className="display-md text-gradient">{s.value}</div>
                <div className="mt-1 font-mono text-xs text-white/50">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
